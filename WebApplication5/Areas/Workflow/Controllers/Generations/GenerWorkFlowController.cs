...
namespace WebApplication5.Areas.Workflow.Controllers
{
    ...
    /// 【流程实例】控制器
    /// </summary>
    public partial class GenerWorkFlowController
    {
        #region 视图
        ...
        {
            var entity = NH.Session.QueryOver<WF_GenerWorkFlow>().Where(x => x.WorkID == WorkID).List().FirstOrDefault();
            if (entity == null)
            {
                entity = new WF_GenerWorkFlow()
                {
                    WorkID = WorkID
                };
            }
